...
考虑到高浊度点可能对应真实水质风险事件，本文不简单删除所有异常峰值，而是区分传感器噪声和风险异常。对于明显不合理的录入错误进行修正或剔除；对于可能反映水质波动的高 $NTU$ 观测值，保留并在风险评价模型中使用。对于原始 Excel 表中少量人工高亮的单元格，本文不直接判定其为异常值，而是保留原始数值，并设置高亮标记变量用于数据质量分析。
